...
def parse_key(key_input: str) -> tuple[str, bool]:
    ...
    if not match:
        raise ValueError(
            f"Invalid key format: {key_input}. Expected format like 'C', 'F#min', 'Bb'"
        )
...
def get_chord_progression(root: str, is_minor: bool) -> dict[str, str]:
    ...
    if is_minor:
        ...
        one_chord, vi, four_chord, five_chord = major_keys[root]
        return {"I": one_chord, "vi": vi, "IV": four_chord, "V": five_chord}
...
def load_template() -> str:
    ...
    """
    template_path = os.path.join(os.path.dirname(__file__), "data", "templates", "basic.songml")
...
def create_project(song_name: str, key_input: str) -> None:
    ...
    try:
        ...
        source_voicings = os.path.join(os.path.dirname(__file__), "data", "chord_voicings.tsv")
...
def main() -> None:
  ...
  %(prog)s "Jazz Standard" Bb""",
    ...
    parser.add_argument(
        "song_name", metavar="SONG_NAME", help="name of the song (used for directory and file)"
    )
    parser.add_argument("key", metavar="KEY", help="musical key (e.g., C, F#min, Bb)")
