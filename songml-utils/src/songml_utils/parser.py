        ...
        if section_match:
            ...
            if section_names_seen and section_name in section_names_seen:
                document.warnings.append(
                    f'Line {line_num + 1}: Duplicate section name "{section_name}"'
                )
...
def _parse_row_group(
    ...
    if cells and cells[0].strip() and cells[0].strip()[0].isdigit():
        ...
        raise ParseError(
            "Section must start with bar-number row (first cell must contain digit)", line_num + 1
        )
...
def _parse_time_signature(time_sig: str, line_number: int) -> int:
    ...
    if denominator not in [4, 8]:
        raise ParseError(
            f"Time signature denominator must be 4 or 8, got {denominator}", line_number
        )
...
def _parse_chord_tokens(
    ...
    while i < len(tokens):
        ...
        if chord_text_part:
            ...
            chord_tokens.append(
                ChordToken(text=chord_text_part, start_beat=current_beat, duration_beats=duration)
            )
